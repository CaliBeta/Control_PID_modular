...
%%  *********  2_calculos realimentacion la planta con controlador **********************
syms('Kp');
syms('Ti');
syms('Td');
syms('S')
...
display ('----------planta1------------')
num_planta_s=1;
den_planta_s=S^2+S-2;
planta=num_planta_s/den_planta_s;
planta=vpa(planta,4);
pretty(planta) 
...
Kp=den_deseada_orden_3(3)+2   
Ti=Kp/den_deseada_orden_3(4)
Td=(den_deseada_orden_3(2)-1)/(Kp)


%% probando los parametros calculados con la funcion step

%------- la planta controlada --------------
num_controlada= [Kp*Td, Kp, Kp/Ti]
den_controlada=[1, 1+Kp*Td, Kp-2, Kp/Ti] 
...
% se debe dar click en simular en el diagrama de bloques de simulink y
% configurar 10 segundos en la simulacion
